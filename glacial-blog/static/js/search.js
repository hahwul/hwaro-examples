(function () {
  ...
  function loadSearchData(cb) {
    ...
    var searchUrl = getBaseUrl() + '/search.json';
    fetch(searchUrl)
      .then(function (r) {
        if (!r.ok) throw new Error('Search data not found');
        return r.json();
      })
  ...
  function search(query) {
    ...
    for (var j = 0; j < results.length; j++) {
      ...
      var url = r.url.startsWith('/') ? (base + r.url) : r.url;
      html += '<a class="search-result-item" href="' + url + '" data-index="' + j + '">'
