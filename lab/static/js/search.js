(function () {
  ...
  function search(query) {
    ...
    if (results.length === 0) {
      resultsEl.innerHTML = '<div style="padding:1rem;color:#8a8f96;font-size:0.88rem;text-align:center">No results for "' + escapeHtml(query) + '"</div>';
      activeIndex = -1;
      return;
    }
    ...
    for (var j = 0; j < results.length; j++) {
      ...
      html += '<a href="' + r.url + '" data-index="' + j + '">'
        + '<strong>' + highlightMatch(r.title, query.trim()) + '</strong>'
        + '<br><small style="color:#5a6068">' + highlightMatch(snippet, query.trim()) + '</small>'
        + '</a>';
    }
  ...
  function updateActive() {
    ...
    for (var i = 0; i < items.length; i++) {
      items[i].style.background = i === activeIndex ? '#f8f8f6' : '';
    }
