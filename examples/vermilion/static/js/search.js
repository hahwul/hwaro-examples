...
(function () {
  ...
  function loadIndex() {
    ...
    fetch(form.getAttribute("data-index"))
      ...
      .catch(function () {
        status.textContent = "The search index could not be loaded. Try the identities listing instead.";
      });
  }
  ...
  function run(query) {
    list.textContent = "";
    if (!query.trim()) {
      status.textContent = "Type to search every case study.";
      return;
    }
    if (!fuse) {
    ...
    if (results.length === 0) {
      status.textContent = "No identities match “" + query + "”.";
      return;
    }
