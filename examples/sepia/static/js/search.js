/* Sepia — page search.
   ...
   as a plain list. All result text goes through textContent, never
...
(function () {
  ...
  fetch(form.getAttribute('data-index'))
    ...
    .catch(function () {
      status.textContent = 'The search index could not be loaded — browse the sheets instead.';
    });
  ...
  function render(results, query) {
    ...
    if (results.length === 0) {
      status.textContent = 'No frames match “' + query + '”.';
      return;
    }
    ...
    results.slice(0, 12).forEach(function (r) {
      ...
      a.textContent = r.item.title || 'Untitled sheet';
      h.appendChild(a);
