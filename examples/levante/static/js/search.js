/* Levante tide-log search (page variant).
   Loads the Fuse.js index hwaro builds at /search.json and renders matches
   as a plain result list. Every result string is written with textContent,
   never innerHTML, so nothing from the index can inject markup. */
(function () {
  ...
  fetch(form.getAttribute("data-index"))
    ...
    .catch(function () {
      status.textContent = "The search index could not be loaded. Try the tide log directly instead.";
    });
  ...
  function render(results, query) {
    ...
    if (results.length === 0) {
      status.textContent = "No entries match “" + query + "”.";
      return;
    }
