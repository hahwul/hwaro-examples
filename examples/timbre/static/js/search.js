/* Timbre — catalog search (page variant).
   Loads the Fuse.js index hwaro builds at /search.json and renders results
   as specimen rows beneath the search field. */
(function () {
  ...
  fetch(form.getAttribute('data-index'))
    ...
    .catch(function () {
      status.textContent = 'The index could not be loaded. Browse the catalog instead.';
    });
  ...
  function render(results, query) {
    ...
    if (results.length === 0) {
      status.textContent = 'No specimen matches “' + query + '”.';
      return;
    }
