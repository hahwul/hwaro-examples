/* Haboob — command palette search. Ctrl+K / Cmd+K (or the header button)
...
(function () {
  ...
  function loadIndex() {
    ...
    fetch(baseUrl + '/search.json')
      ...
      .catch(function () {
        status.textContent = 'The case-file index failed to load.';
      });
  }
  ...
  function render(query) {
    ...
    if (!q) {
      results = [];
      list.innerHTML = '';
      status.textContent = 'Type to search the case files.';
      return;
    }
    if (!fuse) {
    ...
    list.innerHTML = html;
    status.textContent = results.length + (results.length === 1 ? ' result' : ' results') + ' — ↑↓ to move, ↵ to open';
  }
