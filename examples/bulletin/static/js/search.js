/* The Bulletin — page search (fuse_json index built by hwaro).
   Results render as rows in the same ruled list as the archive index. */
(function () {
  ...
  fetch(form.getAttribute('data-index'))
    ...
    .catch(function () {
      status.textContent = 'The search index could not be loaded. Try the archive index instead.';
    });
  ...
  function render(results, query) {
    ...
    status.textContent = results.length + (results.length === 1 ? ' bulletin found.' : ' bulletins found.');
    results.slice(0, 12).forEach(function (r) {
      ...
      slug.className = 'slug';
      slug.textContent = 'From the archive';
