/* Meltemi — search (page variant).
   Loads the Fuse.js index built by hwaro and renders results as
   timeline-style rows, matching the training log's own listings. */
(function () {
  ...
  fetch(form.getAttribute("data-index"))
    ...
    .catch(function () {
      status.textContent = "The index could not be loaded. Try the training log instead.";
    });
  ...
  function render(results, query) {
    ...
    results.slice(0, 10).forEach(function (r) {
      ...
      li.className = "week week-result";

      var a = document.createElement("a");
      a.className = "week-result-link";
      a.href = hrefFor(item);
