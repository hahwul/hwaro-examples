/* Inline grep-style search for the releases index.
   Typing swaps the release list for results in place; clearing
   (or pressing Escape) restores it. All result strings are added
...
(function () {
  ...
  function excerpt(text, query) {
    ...
    var slice = text.slice(start, start + 120).replace(/\s+/g, ' ').trim();
  ...
  function run() {
    ...
    status.textContent = '# ' + hits.length + (hits.length === 1 ? ' match' : ' matches') + ' in ./';
  }
