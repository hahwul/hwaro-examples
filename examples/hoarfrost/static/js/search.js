/* Hoarfrost — search overlay.
   Opens from the header button or the "/" key, closes on Esc or backdrop
   click, restores focus to the trigger, and locks background scroll.
   Results are built from text nodes only — no HTML injection. */
(function () {
  'use strict';
  ...
  var loading = false;
  var active = -1;
  var idleHint = 'Type to search the log — arrows to move, Enter to open.';
  ...
  function run() {
    ...
    status.textContent = hits.length
      ? hits.length + (hits.length === 1 ? ' entry found.' : ' entries found.')
