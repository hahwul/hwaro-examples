/* Drizzle — search (page variant).
   Loads the Fuse.js index built by hwaro and renders results as the
   same experiment cards used across the notebook. */
(function () {
  ...
  fetch(form.getAttribute('data-index'))
    ...
    .catch(function () {
      status.textContent = 'The index could not be loaded. Try the experiment notebook instead.';
    });
  ...
  function render(results, query) {
    ...
    if (results.length === 0) {
      status.textContent = 'Nothing in the notebook matches “' + query + '”.';
      return;
    }
    status.textContent = results.length + (results.length === 1 ? ' experiment found.' : ' experiments found.');
