/* Murmur — search overlay.
   Opens from the header button or the "/" key, closes on Esc or backdrop
   click, keeps focus on the input while open, restores it to the trigger
   on close, and locks background scroll. Results are built from text
...
(function () {
  ...
  var loading = false;
  var active = -1;
  var idleHint = 'Type to search every dispatch Murmur has logged.';
  ...
  function run() {
    ...
    status.textContent = hits.length
      ? hits.length + (hits.length === 1 ? ' dispatch found.' : ' dispatches found.')
      : 'Nothing logged matches “' + q + '”.';
