/* Rime — search overlay. Opens from the header button or the "/" key,
...
(function () {
  ...
  var active = -1;
  var lastFocus = null;
  var idle = "Type to search every ascent and page in the log.";
  ...
  function run(q) {
    ...
    if (status) {
      status.textContent = results.length
        ? results.length + (results.length === 1 ? " line matches." : " lines match.")
