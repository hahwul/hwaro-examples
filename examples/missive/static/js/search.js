/* Missive — page search (fuse_json index built by hwaro).
   Results render as rows in the same ruled column as the issue archive. */
(function () {
  ...
  fetch(form.getAttribute("data-index"))
    ...
    .catch(function () {
      status.textContent = "The search index could not be loaded. Try the issue archive instead.";
    });
  ...
  function render(results, query) {
    ...
    if (results.length === 0) {
      status.textContent = "Nothing filed matches “" + query + "”.";
      return;
    }
    status.textContent = results.length + (results.length === 1 ? " specimen found." : " specimens found.");
