/* Gust — command palette search. Ctrl+K / Cmd+K (or the header button)
   opens a centered modal; arrow keys move the selection, Enter opens the
   entry, Esc closes and returns focus to the trigger. */
(function () {
  'use strict';
  ...
  function loadIndex() {
    ...
    fetch(baseUrl + '/search.json')
      ...
      .catch(function () {
        status.textContent = 'The station index failed to load.';
      });
  }
  ...
  function render(query) {
    ...
    if (!q) {
      results = [];
      list.innerHTML = '';
      status.textContent = 'Type to search the station log.';
      return;
    }
    if (!fuse) {
      status.textContent = 'Warming up the index…';
      return;
    }
    results = fuse.search(q).slice(0, 8);
    if (results.length === 0) {
      list.innerHTML = '';
      status.textContent = 'No copy on “' + q + '”.';
      return;
    }
    var html = '';
    ...
    list.innerHTML = html;
    status.textContent = results.length + (results.length === 1 ? ' contact' : ' contacts') + ' — ↑↓ to move, ↵ to open';
  }
