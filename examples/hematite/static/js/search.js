/* Hematite — command palette search (Ctrl+K / Cmd+K).
...
(function () {
  ...
  function load() {
    ...
    fetch(base + "/search.json")
      ...
      .catch(function () {
        if (status) status.textContent = "The search index would not load — the sidebar still works.";
      });
  }
  ...
  function open() {
    ...
    input.value = "";
    render([]);
    if (status) status.textContent = "Type to search every page in the docs.";
    load();
    input.focus();
  }
  ...
  function run(q) {
    ...
    render(results);
    if (!status) return;
    if (!query) status.textContent = "Type to search every page in the docs.";
    else if (!results.length) status.textContent = "Nothing in the docs matches “" + query + "”.";
