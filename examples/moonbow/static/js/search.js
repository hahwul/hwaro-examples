/* Moonbow — command-palette search.
   Ctrl+K / Cmd+K toggles a centered modal; arrows move the aria-selected
   row, Enter opens it, Esc closes and returns focus to the trigger.
   Fuse.js loads from the CDN in footer.html. Every result string is written
   with textContent — never innerHTML — so indexed content cannot inject markup. */
(function () {
  'use strict';
  ...
  var openers = [document.getElementById('search-open'), document.getElementById('search-open-2')];
  ...
  function open() {
    ...
    openers.forEach(function (b) { if (b) b.setAttribute('aria-expanded', 'true'); });
    loadIndex();
    input.value = '';
    list.textContent = '';
  ...
  function close() {
    ...
    openers.forEach(function (b) { if (b) b.setAttribute('aria-expanded', 'false'); });
