(function () {
  ...
  var overlay = document.getElementById("search-dialog");
  ...
  function render(query) {
    var q = query.trim();
    results.innerHTML = "";
    if (!q) { status.textContent = "Type to search the sighting log."; status.hidden = false; return; }
    if (!fuse) { status.textContent = "Loading the log…"; status.hidden = false; return; }
    var hits = fuse.search(q).slice(0, 8);
    if (!hits.length) {
      status.textContent = "No sightings match “" + q + "”.";
      status.hidden = false;
      return;
    }
    status.hidden = true;
    hits.forEach(function (hit) {
      ...
      a.innerHTML =
        '<span class="search-hit-title">' + escapeHtml(item.title || "Untitled") + "</span>" +
        '<span class="search-hit-excerpt">' + escapeHtml(body) + "</span>";
      li.appendChild(a);
