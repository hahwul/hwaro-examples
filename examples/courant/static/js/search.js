/* The Courant — search overlay. Opens from the header button or the "/" key,
   closes on Esc, returns focus to the trigger, locks nothing else on the page.
   Fuse.js loads from the CDN in footer.html; every result string is inserted
...
(function () {
  ...
  var active = -1;
  var lastFocus = null;
  var idle = "Type to search every notice in the archive.";
  ...
  function run(q) {
    ...
    if (status) {
      status.textContent = results.length
        ? results.length + (results.length === 1 ? " notice matches." : " notices match.")
