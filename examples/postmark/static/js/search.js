/* Postmark -- search overlay. Opens from the header button or the "/" key,
   closes on Esc, and returns focus to whichever control opened it. Fuse.js
   loads from the CDN in footer.html; every result string is inserted with
   textContent, never innerHTML, so nothing in the index can inject markup. */
(function () {
  ...
  var active = -1;
  var lastFocus = null;
  var idle = "Type to search every issue on file.";
  ...
  function load() {
    ...
    fetch(base + "/search.json")
      ...
      .catch(function () {
        if (status) status.textContent = "The search index would not load. The nav above still works.";
      });
  }
  ...
  function run(q) {
    ...
    if (status) {
      status.textContent = results.length
        ? results.length + (results.length === 1 ? " issue matches." : " issues match.")
