...
(function () {
  ...
  function loadIndex() {
    ...
    fetch(form.getAttribute("data-index"))
      ...
      .catch(function () {
        status.textContent = "The search index could not be loaded. Try the log listing instead.";
      });
  }
  ...
  function run(query) {
    ...
    if (!fuse) {
      status.textContent = "Warming up the comb…";
      return;
    }
    var results = fuse.search(query).slice(0, 8);
    if (results.length === 0) {
      status.textContent = "No cells in the comb match “" + query + "”.";
      return;
    }
    status.textContent = results.length + (results.length === 1 ? " cell found." : " cells found.");
    results.forEach(function (r) {
      ...
      title.textContent = r.item.title || "Untitled entry";
