(function () {
  ...
  fetch(form.getAttribute('data-index'))
    ...
    .catch(function () {
      status.textContent = 'The index could not be loaded.';
    });
  ...
  function snippet(text) {
    ...
    if (clean.length > 150) return clean.slice(0, 150) + '…';
    return clean;
  }
  ...
  function render(results, query) {
    ...
    if (results.length === 0) {
      status.textContent = 'No results match “' + query + '”.';
      return;
    }
    ...
    results.slice(0, 12).forEach(function (r) {
      ...
      textNode.className = 'search-hit-snippet';
      textNode.textContent = snippet(item.content);
