/* Revue — overlay search. The header button or the "/" key opens a modal
   dialog; typing searches the run of essays via Fuse.js. Arrow keys move
   the selection, Enter opens it, Esc closes and returns focus to the
   trigger. Background scroll is locked while the overlay is open. */
(function () {
  'use strict';
  ...
  function render(query) {
    ...
    if (!q) {
      list.innerHTML = '';
      status.textContent = 'Type to search every essay in print.';
      return;
    }
    ...
    list.innerHTML = html;
    status.textContent = results.length + (results.length === 1 ? ' match' : ' matches') + ' — arrow keys to move, enter to open';
  }
