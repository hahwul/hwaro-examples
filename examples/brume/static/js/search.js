/* Brume — inline "find a moss" search.
   Typing in the header field swaps the page's content for a result list
   in place; clearing the field (or Esc) restores it. */
(function () {
  'use strict';

  var input = document.getElementById('find-moss');
  ...
  function render(query) {
    ...
    if (status) {
      status.textContent = hits.length === 0
        ...
        : hits.length + (hits.length === 1 ? ' patch answers' : ' patches answer') +
          ' to “' + query + '”.';
    }
    if (hits.length === 0) {
      results.innerHTML =
        '<li class="search-empty">No moss by that name on this wall — try “slurry”, “stone”, or a genus.</li>';
      return;
    }
