/* Inline catalog search. The rule index doubles as the search surface:
   typing swaps the server-rendered index for matching results in place,
   and clearing the field (or Escape) restores it. Results are built with
   textContent only, never raw HTML. */
(function () {
  "use strict";
  ...
  var list = document.getElementById("rule-index-list");
  var results = document.getElementById("rule-search-results");
  var status = document.getElementById("rule-search-status");
  ...
  function run() {
    ...
    status.textContent = hits.length + (hits.length === 1 ? " match" : " matches") + " in the catalog.";
  }
