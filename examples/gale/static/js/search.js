/* Gale — search overlay. Opens from the header button or the "/" key,
...
(function () {
  ...
  var items = [];
  var active = -1;
  var idleText = "Type to search every note in the journal.";
  ...
  function load() {
    ...
    fetch(base + "/search.json")
      ...
      .catch(function () {
        if (status) status.textContent = "The index would not load — try the field-notes page instead.";
      });
  }
  ...
  function run(q) {
    ...
    else if (!results.length) status.textContent = "Nothing in the log matches “" + query + "”.";
    else status.textContent = results.length + (results.length === 1 ? " note found." : " notes found.");
  }
