/* Dolomite specification search (page variant).
   Loads the Fuse.js index that hwaro builds at /search.json and renders
   matches as a plain list. Every result string is written with
...
(function () {
  ...
  fetch(form.getAttribute('data-index'))
    ...
    .catch(function () {
      status.textContent = 'The search index could not be loaded. Try the specification contents instead.';
    });
  ...
  function render(results, query) {
    ...
    if (results.length === 0) {
      status.textContent = 'No section matches “' + query + '”.';
      return;
    }
    status.textContent = results.length + (results.length === 1 ? ' section found.' : ' sections found.');
