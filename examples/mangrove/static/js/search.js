/* Mangrove — command palette search (Ctrl+K / Cmd+K).
...
(function () {
  ...
  function open() {
    ...
    input.value = "";
    render([]);
    if (status) status.textContent = "Type to search every chapter of the handbook.";
    load();
    input.focus();
  }
  ...
  function run(q) {
    ...
    render(results);
    if (!status) return;
    if (!query) status.textContent = "Type to search every chapter of the handbook.";
    else if (!results.length) status.textContent = "Nothing found for “" + query + "”.";
