/* Fogbow — journal search (page variant).
   Loads the Fuse.js index that hwaro builds and renders matches
   as contents rows, the same anatomy as the table of entries.
   All result text is inserted with textContent — never innerHTML. */
(function () {
  ...
  fetch(form.getAttribute('data-index'))
    ...
    .catch(function () {
      status.textContent = 'The index could not be opened. Try the table of entries instead.';
    });
