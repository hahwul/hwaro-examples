/* Inline manual search for Anthracite.
   The rules rail doubles as the search surface: typing swaps the rule list
   for matching results in place, and clearing the field (or Escape)
   restores it. Every result string is inserted as a text node, never as
   raw HTML. */
(function () {
  'use strict';

  var input = document.getElementById('rulesnav-input');
  if (!input || typeof Fuse === 'undefined') { return; }

  var list = document.getElementById('rulesnav-list');
  var results = document.getElementById('rulesnav-results');
  var status = document.getElementById('rulesnav-status');
  ...
  function run() {
    ...
    status.textContent = hits.length + (hits.length === 1 ? ' match' : ' matches') + ' in the manual.';
  }
