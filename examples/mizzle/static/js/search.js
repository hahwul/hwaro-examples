/* Mizzle — search (page variant).
   Loads the Fuse.js index hwaro builds and renders matches as quiet
   rows hanging from the same scroll rule as the session list. */
(function () {
  ...
  fetch(form.getAttribute('data-index'))
    ...
    .catch(function () {
      status.textContent = 'The index could not be loaded. Try the session list instead.';
    });
  ...
  function render(results, query) {
    ...
    status.textContent = results.length + (results.length === 1 ? ' session found.' : ' sessions found.');
