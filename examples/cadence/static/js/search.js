...
(function () {
  ...
  fetch(form.getAttribute('data-index'))
    ...
    .catch(function () {
      status.textContent = 'The index could not be loaded. Browse the episodes instead.';
    });
  ...
  function render(results, query) {
    ...
    if (results.length === 0) {
      status.textContent = 'No episode matches “' + query + '”.';
      return;
    }
    status.textContent = results.length + (results.length === 1 ? ' episode found.' : ' episodes found.');
