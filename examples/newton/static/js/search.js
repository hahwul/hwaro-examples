(function () {
  ...
  function loadIndex() {
    ...
    fetch(url)
      ...
      .catch(function () { status.textContent = "The catalog index could not be loaded."; });
  }
  ...
  function render(query) {
    ...
    results.innerHTML = "";
    if (!q) {
      status.textContent = "Type to search cultivars and pages — results update as you go. Press Esc to close.";
      status.hidden = false;
      return;
    }
    if (!fuse) { status.textContent = "Loading the catalog…"; status.hidden = false; return; }
    var hits = fuse.search(q).slice(0, 8);
    if (!hits.length) {
      status.textContent = "Nothing in stock matches “" + q + "”.";
      status.hidden = false;
      return;
    }
    status.hidden = true;
