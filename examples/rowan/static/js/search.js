(function () {
  ...
  var trigger = document.getElementById("search-trigger-top");
  ...
  function render(results) {
    ...
    results.forEach(function (r, i) {
      var page = r.item;
      var rubric = page.url.replace(/^\/|\/$/g, "").split("/")[0] || "codex";
      ...
      a.innerHTML =
        '<span class="palette-result-family">' + escapeHtml(rubric) + "</span>" +
  ...
  input.addEventListener("keydown", function (event) {
    if (event.key === "ArrowDown") {
      ...
      if (items.length) { selected = (selected + 1) % items.length; markSelected(); }
    } else if (event.key === "ArrowUp") {
      event.preventDefault();
      if (items.length) { selected = (selected - 1 + items.length) % items.length; markSelected(); }
