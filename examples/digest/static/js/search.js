...
(function () {
  ...
  function loadIndex() {
    ...
    fetch(form.getAttribute("data-index"))
      ...
      .catch(function () {
        status.textContent = "The search index could not be loaded. Try the archive listing instead.";
      });
  }
  ...
  function run(query) {
    ...
    if (results.length === 0) {
      status.textContent = "No plates match “" + query + "”.";
      return;
    }
    status.textContent = results.length + (results.length === 1 ? " plate found." : " plates found.");
    results.forEach(function (r) {
      ...
      title.textContent = r.item.title || "Untitled brief";
